...
module MailControllerTestHelper
  ...
  def deliver(action, mail_params = {}, send_params = {})
    SignupMailer.dispatch_and_deliver(action, { :from => "no-reply@webapp.com", :to => "recepient@person.com" }.merge(mail_params), send_params)
...
describe SignupMailer, "#notify_on_event email template" do
  ...
  it "includes welcome phrase in email text" do
    ...
    # SignupMailer.dispatch_and_deliver(:notify_on_event, {}, { :name => "merb-mailer user" })
